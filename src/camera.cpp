...
Camera::Camera(int width, int height, glm::vec3 position, glm::vec3 direction,
  ...
  if (m_proj_mode == PERSPECTIVE)
    m_proj =
        glm::perspective(glm::radians(m_FOV), (float)m_width / (float)m_height,
                         m_nearPlane, m_farPlane);
  else
    m_proj = glm::ortho(glm::radians(m_FOV), (float)width / (float)height,
                        m_nearPlane, m_farPlane);
  m_proj[1][1] *= -1;
}
...
void Camera::updateCam(Shader &shader) {
  ...
  float sens = 0.0005f;
  m_direction.x = cos(glm::radians(yaw)) * cos(glm::radians(pitch)) * sens;
  m_direction.y = sin(glm::radians(pitch)) * sens;
  m_direction.z = sin(glm::radians(yaw)) * cos(glm::radians(pitch)) * sens;
  ...
  if (m_proj_mode == PERSPECTIVE)
    ...
    m_proj = glm::ortho(glm::radians(m_FOV), (float)m_width / (float)m_height,
                        m_nearPlane, m_farPlane);
  m_proj[1][1] *= -1;
...
void Camera::updateCamSkyBox(Shader &shader) {
  ...
  m_view = glm::mat4(1.0f);
  m_view = glm::mat4(
      glm::mat3(glm::lookAt(m_position, m_position + m_direction, camUp)));
  m_proj = glm::mat4(1.f);
  if (m_proj_mode == PERSPECTIVE)
    m_proj =
        glm::perspective(glm::radians(m_FOV), (float)m_width / (float)m_height,
