...
extern std::unordered_map<std::string, Shader> g_ShaderCache;

std::string extract_string_beetween(const std::string& input, char start_symbol, char end_symbol) {
  ...
  if (start_index != std::string::npos && end_index != std::string::npos) {
    ...
    std::cout << "ERROR [ ShaderLoader::extract_first_between ]: Start '" << start_symbol << "' or end symbol '"
              << end_symbol << "' not found" << std::endl;
  }
  ...
  if (shaderFile.is_open()) {
    ...
    while (getline(shaderFile, line)) {
      if (line.find(includeKey) == 0) {
        std::string includePath = m_rootDir + '/' + extract_string_beetween(line.substr(includeKey.length()), '"', '"');
...
void Shader::HotReload() {
  Delete();
  ID = ShaderLoader().GenerateShaderProgramID(m_VertexSrcPath, m_FragmentSrcPath);
}

void Shader::setUniformInt(const std::string name, int a) { glUniform1i(glGetUniformLocation(ID, name.c_str()), a); }
...
void Shader::setUniformFloat3(const std::string name, float a, float b, float c) {
...
void Shader::setUniformFloat4(const std::string name, float a, float b, float c, float d) {
...
void Shader::setUniformMatrix3f(const std::string name, glm::mat3 mtrx) {
  glUniformMatrix3fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, glm::value_ptr(mtrx));
}
...
// SHADER_LOADER
Shader ShaderLoader::loadShaderProgram(const std::string& VertexPath, const std::string& FragmentPath) {
  unsigned int ID = glCreateProgram();
  unsigned int VertexSh = CompileShader(GL_VERTEX_SHADER, GetShaderSrcFromFile(VertexPath));
  unsigned int FragmentSh = CompileShader(GL_FRAGMENT_SHADER, GetShaderSrcFromFile(FragmentPath));
...
unsigned int ShaderLoader::GenerateShaderProgramID(const std::string& VertexPath, const std::string& FragmentPath) {
  unsigned int ID = glCreateProgram();
  unsigned int VertexSh = CompileShader(GL_VERTEX_SHADER, GetShaderSrcFromFile(VertexPath));
  unsigned int FragmentSh = CompileShader(GL_FRAGMENT_SHADER, GetShaderSrcFromFile(FragmentPath));
...
unsigned int ShaderLoader::CompileShader(const GLenum type, const std::string& source) {
